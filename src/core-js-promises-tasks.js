...
function getPromiseResult(source) {
  return new Promise((resolve) => {
    source.then(() => resolve('success')).catch(() => resolve(resolve('fail')));
  });
}
...
async function getAllResult(promises) {
  return Promise.allSettled(promises).then((promisesArr) =>
    promisesArr.map((el) => (el.status === 'fulfilled' ? el.value : null))
  );
}
...
function queuPromises(promises) {
  ...
  promises.forEach((curPromise) => {
    tempPromise = tempPromise.then((tempPromiseResult) =>
      curPromise.then(
        (curPromiseResult) => tempPromiseResult + curPromiseResult
      )
    );
  });
